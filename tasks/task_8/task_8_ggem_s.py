...
import sys
import json
sys.path.append(os.path.abspath('Radical_AI//mission-quizify-github-taiwo//mission-quizify'))
from tasks.task_3.task_3_s import DocumentProcessor
from tasks.task_4.task_4_ggem_s import EmbeddingClient
from tasks.task_5.task_5_ggem_s import ChromaCollectionCreator
...
class QuizGenerator:
    ...
    def generate_quiz(self) -> list:
        ...
        for _ in range(self.num_questions):
            ...
            # generator = QuizGenerator(topic_input, questions, chroma_creator) # Initialize
            ...
            try:
                ...
                st.write("Failed to decode question JSON.")
            ...
            if self.validate_question(question):
                ...
                # Note: retry a couple of times  
                # initialize i
                i = 1 
                while i <= 1:
                    # generate questions
                    question_try = self.generate_question_with_vectorstore()  
                    
                    # Convert the JSON String to a dictionary
                    try:
                        # Convert the JSON String to a dictionary
                        question_j = json.loads(question_try)
                    except json.JSONDecodeError:
                        print("Failed to decode question JSON.")
                        continue  # Skip this iteration if JSON decoding fails
             
                    # validate; if true, add to question bank
                    if self.validate_question(question_j):
                        self.question_bank.append(question)
                        break
                    # else, increase i
                    else:
    ...
    def validate_question(self, question: dict) -> bool:
        ...
        This method checks if the provided question (as a dictionary) is unique based on its text content compared to previously 
        generated questions stored in `question_bank`. The goal is to ensure that no duplicate questions are added to the quiz.
        ...
        if question == None:
            ...
            question_text = next(iter((question.items())))[1]
            ...
            # and compare their texts to the current question's text.
            for k in range(len(self.question_bank)):
                question_bank_text_k = next(iter((self.question_bank[k].items())))[1]
                # st.write(question_bank_text_k)
                
                if question_text.casefold() == question_bank_text_k.casefold():
